...
GTKv3_CONF=$(wildcard ./gtk/gtkrc-3.0/*)

GTK_CONFS=$(GTKv1_CONF) $(GTKv2_CONF) $(GTKv3_CONF)
GTK_CONFS_BUILD=\
	$(patsubst ./gtk/%,$(BUILD)/.%,$(GTKv1_CONF)) \
	$(patsubst ./gtk/%,$(BUILD)/.%,$(GTKv2_CONF)) \
	$(patsubst ./gtk/gtkrc-3.0/%,$(BUILD)/.config/gtkrc-3.0/%,$(GTKv3_CONF))

.PHONY: gtkrc

gtk: $(GTK_CONFS_BUILD)

$(BUILD)/.config/gtkrc-3.0/%: ./gtk/gtkrc-3.0/%
	[ -d $(BUILD)/.config/gtkrc-3.0 ] || mkdir -p $(BUILD)/.config/gtkrc-3.0
	cp $? $@
